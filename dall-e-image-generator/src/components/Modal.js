...
const Modal = ({ setModalOpen, setSelectedImage, selectedImage, generateVariations }) => {
  ...
  const checkSize = () => {
    if (ref.current.width === 256 && ref.current.height === 256) {
      ...
      setError('Error: Improper dimensions, Choose a 256 x 256 image file')
    }
  }
  return (
    <div className="modal">
      <div className="xButton" onClick={closeModal}>X</div>
      ...
      {error && <button onClick={closeModal}>Close this window and try again</button>}
    </div>
  )
}
